...
from shared.domain.gen_features import *
from shared.domain.gen_labels_highlow import generate_labels_highlow, generate_labels_highlow2
from shared.domain.gen_labels_topbot import generate_labels_topbot, generate_labels_topbot2
...
def predict_feature_set(df, fs, config, model_store: ModelStore) -> Tuple[pd.DataFrame, list]:
    ...
    for label in labels:
        for model_config in algorithms:
            ...
            if algo_type == "gb":
                from shared.domain.classifier_gb import predict_gb
                df_y_hat = predict_gb(model_pair, train_df, model_config)
            elif algo_type == "nn":
                from shared.domain.classifier_nn import predict_nn
                df_y_hat = predict_nn(model_pair, train_df, model_config)
            elif algo_type == "lc":
                from shared.domain.classifier_lc import predict_lc
                df_y_hat = predict_lc(model_pair, train_df, model_config)
            elif algo_type == "svc":
                from shared.domain.classifier_svc import predict_svc
...
def train_feature_set(df, fs, config) -> dict:
    ...
    for label in labels:
        for model_config in algorithms:
            ...
            if algo_type == "gb":
                from shared.domain.classifier_gb import train_gb
                model_pair = train_gb(df_X, df_y, model_config)
                models[score_column_name] = model_pair
            elif algo_type == "nn":
                from shared.domain.classifier_nn import train_nn
                model_pair = train_nn(df_X, df_y, model_config)
                models[score_column_name] = model_pair
            elif algo_type == "lc":
                from shared.domain.classifier_lc import train_lc
                model_pair = train_lc(df_X, df_y, model_config)
                models[score_column_name] = model_pair
            elif algo_type == "svc":
                from shared.domain.classifier_svc import train_svc
...
async def output_feature_set(df, fs: dict, config: dict, model_store: ModelStore) -> None:
    from shared.notifiers.notifier_trades import trader_simulation
