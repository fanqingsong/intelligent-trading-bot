...
def generate_majority_vote(df, config):
    """Majority vote across per-algorithm buy/sell signal columns.

    Config:
      buy_columns: list[str]
      sell_columns: list[str]
      min_votes: int (default 2)
      names: [buy_signal_vote, sell_signal_vote, vote_label]
    """
    buy_columns = config.get("buy_columns") or []
    sell_columns = config.get("sell_columns") or []
    if not buy_columns or not sell_columns:
        raise ValueError("majority_vote requires non-empty buy_columns and sell_columns")

    names = config.get("names") or ["buy_signal_vote", "sell_signal_vote", "vote_label"]
    buy_out, sell_out = names[0], names[1]
    label_out = names[2] if len(names) > 2 else "vote_label"
    min_votes = int(config.get("min_votes", 2))

    missing = [c for c in buy_columns + sell_columns if c not in df.columns]
    if missing:
        raise ValueError(f"majority_vote missing columns: {missing}")

    buy_count = df[buy_columns].astype(bool).sum(axis=1)
    sell_count = df[sell_columns].astype(bool).sum(axis=1)
    df[buy_out] = buy_count >= min_votes
    df[sell_out] = sell_count >= min_votes

    def _label(b: bool, s: bool) -> str:
        if b and not s:
            return "BUY"
        if s and not b:
            return "SELL"
    ...
    df[label_out] = [
        _label(bool(b), bool(s)) for b, s in zip(df[buy_out].tolist(), df[sell_out].tolist())
    ]
    return df, [buy_out, sell_out, label_out]
